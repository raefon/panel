...
@section('title')
    {{ $cluster->name }}: Allocations
@endsection

@section('content-header')
    <h1>{{ $cluster->name }}<small>Control allocations available for servers on this cluster.</small></h1>
    <ol class="breadcrumb">
    ...
    <div class="col-xs-12">
        <div class="nav-tabs-custom nav-tabs-floating">
            <ul class="nav nav-tabs">
                ...
                <li class="active"><a href="{{ route('admin.clusters.view.allocation', $cluster->id) }}">Allocation</a></li>
        ...
        <div class="box box-primary">
            ...
            <div class="box-body table-responsive no-padding" style="overflow-x: visible">
                <table class="table table-hover" style="margin-bottom:0;">
                    ...
                    @foreach($cluster->allocations as $allocation)
                        <tr>
            ...
            @if($cluster->allocations->hasPages())
                <div class="box-footer text-center">
                    {{ $cluster->allocations->render() }}
                </div>
            @endif
        </div>
    </div>
    <div class="col-sm-4">
        <form action="{{ route('admin.clusters.view.allocation', $cluster->id) }}" method="POST">
...
<div class="modal fade" id="allocationModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.clusters.view.allocation.removeBlock', $cluster->id) }}" method="POST">
...
@section('footer-scripts')
    ...
    $('button[data-action="deallocate"]').click(function (event) {
        ...
        swal({
            ...
            $.ajax({
                method: 'DELETE',
                url: '/admin/clusters/view/' + {{ $cluster->id }} + '/allocation/remove/' + allocation,
    ...
    function sendAlias(element) {
        ...
        $.ajax({
            method: 'POST',
            url: '/admin/clusters/view/' + {{ $cluster->id }} + '/allocation/alias',
            headers: { 'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content') },
            data: {
                alias: element.val(),
    ...
    function deleteSelected() {
        ...
        if (selectedItems.length !== 0) {
            ...
            swal({
                ...
                $.ajax({
                    method: 'DELETE',
                    url: '/admin/clusters/view/' + {{ $cluster->id }} + '/allocations',
                    headers: {'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')},
                    data: JSON.stringify({
