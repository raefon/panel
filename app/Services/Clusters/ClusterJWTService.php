...
class ClusterJWTService
{
    ...
    /**
     * Generate a new JWT for a given cluster.
     */
    public function handle(Cluster $cluster, ?string $identifiedBy, string $algo = 'md5'): Plain
    {
        $identifier = hash($algo, $identifiedBy);
        $config = Configuration::forSymmetricSigner(new Sha256(), InMemory::plainText($cluster->getDecryptedKey()));

        $builder = $config->builder(new TimestampDates())
            ->issuedBy(config('app.url'))
            ->permittedFor($cluster->getConnectionAddress())
