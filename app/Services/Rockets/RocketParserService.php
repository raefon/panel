...
use Kubectyl\Exceptions\Service\InvalidFileUploadException;

class RocketParserService
{
    /**
     * Takes an uploaded file and parses out the rocket configuration from within.
     *
     ...
     */
    public function fillFromParsed(Rocket $model, array $parsed): Rocket
    {
    ...
    /**
     * Converts a PTDL_V1 rocket into the expected PTDL_V2 rocket format. This just handles
    ...
    {
        if (Arr::get($parsed, 'meta.version') === Rocket::EXPORT_VERSION) {
            return $parsed;
        }

        // Maintain backwards compatability for rockets that are still using the old single image
        // string format. New rockets can provide an array of Docker images that can be used.
