...
class RocketConfigurationService
{
    /**
     * RocketConfigurationService constructor.
     */
    ...
    /**
     * Return an Rocket file to be used by the Daemon.
     */
    public function handle(Server $server): array
    {
        $configs = $this->replacePlaceholders(
            $server,
            json_decode($server->rocket->inherit_config_files)
        );

        return [
            'startup' => $this->convertStartupToNewFormat(json_decode($server->rocket->inherit_config_startup, true)),
            'stop' => $this->convertStopToNewFormat($server->rocket->inherit_config_stop),
            'configs' => $configs,
        ];
    }
     ...
     *
     * For most rockets, this ends up just being a command sent to the server console, but
    ...
    protected function replacePlaceholders(Server $server, object $configs): array
    {
        ...
        // can property map the rocket placeholders to values.
        ...
        foreach ($configs as $file => $data) {
            ...
            // or other JSON data for the rocket. This should probably be blocked at the time
            // of rocket creation/update, but it isn't so this check will at least prevent a
    ...
    /**
     * Replaces the legacy modifies from rockets with their new counterpart. The legacy Daemon would
    ...
    protected function matchAndReplaceKeys(mixed $value, array $structure): mixed
    {
        ...
        foreach ($matches['key'] as $key) {
            ...
            // that. For example, the Spigot rocket uses "config.docker.interface" to identify the Docker
